...
Quaternary hydrides follow the formula A_l B_m FeH_n where:
- A and B are different electropositive metals (Group I, II, or III), with A ≠ B
- Fe has fixed valence of +2
...
def count_excess_electrons(A: str, l: int, B: str, m: int, n: int, h: int) -> float:
    """
    Count excess valence electrons in quaternary composition A_l B_m Fe_n H_h.
    
    Positive excess = potential hydride
    Formula: n_excess = valence(A)*l + valence(B)*m + valence(Fe)*n + valence(H)*h
    Note: H has valence -1, so it reduces the total
    """
    valence_A = VALENCE_ELECTRONS[A]
    valence_B = VALENCE_ELECTRONS[B]
    
    excess = (valence_A * l + valence_B * m + FE_VALENCE * n + H_VALENCE * h)
...
def search_quaternary_hydrides(
    ...
    """
    Search for potential quaternary hydride compositions A_l B_m Fe_n H_h.
    
    Args:
        group_ab: List of elements for positions A and B (must be different, A ≠ B)
    ...
    print("Searching for potential quaternary hydrides (A-B-Fe-H)...")
    print(f"Constraint: A ≠ B")
    print(f"Variable: A_l B_m Fe_n H_h with Fe valence = {FE_VALENCE}, H valence = {H_VALENCE}")
    ...
    for A in group_ab:
        for B in group_ab:
            # Constraint: A must be different from B
            if A == B:
                continue
                ...
                for m in range(1, max_atoms):
                    for n in range(1, max_atoms):
                        for h in range(1, max_atoms):
                            ...
                            excess = count_excess_electrons(A, l, B, m, n, h)
...
def save_compositions(compositions: List[Dict], output_file: str = "quaternary_hydride_compositions.json"):
...
def main():
    """Main execution."""
    print("="*70)
    print("QUATERNARY HYDRIDE COMPOSITION SEARCH (A-B-Fe-H)")
    print("="*70)
    ...
    save_compositions(compositions, "quaternary_hydride_compositions.json")
