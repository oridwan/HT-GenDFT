#!/usr/bin/env python3
"""
Compute DFT energy_above_hull for origin-flow electride candidates only.
...
Output: dft_stability_candidates.json with DFT-level hull analysis for the shortlisted candidates


python3 origin_flow/compute_dft_e_hull_origin.py \
  --vasp-jobs "$PWD/VASP-out-Boron" \
  --candidate-csv "$PWD/VASP-out-Boron/electride_analysis_candidates-Strict.csv" \
  --prescreen-results "$PWD/prescreen_new/VASP_JOBS_Boron/prescreening_stability.json" \
  --mattersim-cache "$PWD/prescreen_new/VASP_JOBS_Boron/mp_mattersim.json" \
  --output dft_stability_candidates.json \
  --hull-threshold 0.1 \
...
def load_candidate_structure_ids(csv_path):
    """
    Load structure IDs from a candidate CSV.

    Accepts either a `formula` column (preferred in this repo) or `structure_id`.
    """
...
def resolve_path_with_fallbacks(path_str, base_dir=None, fallback_dirs=None):
    """Resolve a possibly-relative path against a base dir and optional fallback dirs."""
    if path_str is None:
        return None
    ...
    for fallback_dir in fallback_dirs or []:
        ...
        candidates.append(fallback_dir / path.name)
...
def build_origin_prescreen_fallback_dirs(vasp_jobs):
    """
    Build fallback directories for origin-flow prescreen artifacts.

    Example:
    - VASP-out-Boron -> prescreen_new/VASP_JOBS_Boron
    - generic fallback -> prescreen/VASP_JOBS
    """
    vasp_jobs = Path(vasp_jobs)
    fallback_dirs = []

    if vasp_jobs.name.startswith('VASP-out-'):
        suffix = vasp_jobs.name[len('VASP-out-'):]
        fallback_dirs.append(vasp_jobs.parent / 'prescreen_new' / f'VASP_JOBS_{suffix}')

    fallback_dirs.append(vasp_jobs.parent / 'prescreen_new' / 'VASP_JOBS')
    fallback_dirs.append(vasp_jobs.parent / 'prescreen' / 'VASP_JOBS')
...
def main():
    parser = argparse.ArgumentParser(
        description="Compute DFT energy_above_hull for origin-flow electride candidates only"
    )
    parser.add_argument(
        '--vasp-jobs',
        type=str,
    ...
    parser.add_argument(
        '--output',
        type=str,
        default='dft_stability_candidates.json',
        help="Output JSON file (default: dft_stability_candidates.json)"
    )
    parser.add_argument(
        '--prescreen-results',
        type=str,
    ...
    prescreen_fallback_dirs = build_origin_prescreen_fallback_dirs(vasp_jobs)
    candidate_csv_path = resolve_path_with_fallbacks(
        args.candidate_csv,
        base_dir=vasp_jobs,
    )
    ...
    # Load workflow database early so the candidate filter can constrain the chemsys set.
    ...
    if not candidate_csv_path.exists():
        ...
        print("This origin-specific script requires a candidate CSV to avoid processing all completed structures.")
        return 1
    ...
    candidate_chemsys = set()
    for struct_id in candidate_structure_ids:
        ...
        candidate_chemsys.add(chemsys)

    print(f"Unique candidate chemical systems: {len(candidate_chemsys)}")
    print()

    # Set up cache paths (single global cache files)
    mp_mattersim_cache_path = resolve_path_with_fallbacks(
        args.mattersim_cache,
        base_dir=vasp_jobs,
        fallback_dirs=prescreen_fallback_dirs,
    )
    ...
    try:
        ...
        # Restrict DFT cache pre-population to the candidate chemical systems only.
        mattersim_chemsys = sorted(candidate_chemsys)

        print(f"Loaded MatterSim cache: {len(mattersim_cache)} reference phases")
        print(f"Candidate chemical systems to pre-populate: {len(mattersim_chemsys)}")
        print(f"  {sorted(mattersim_chemsys)}")
        print()
        
        # Pre-populate DFT cache with GGA energies for the candidate chemical systems only.
        print("Pre-populating DFT cache with GGA energies for candidate chemical systems...")
        print("(This ensures DFT and MatterSim hulls use the same reference phase set)")
        print()
    ...
    if args.prescreen_results:
        prescreen_path = resolve_path_with_fallbacks(
            args.prescreen_results,
            base_dir=vasp_jobs,
            fallback_dirs=prescreen_fallback_dirs,
        )
    ...
    output_data = {
        'summary': {
            ...
            'reference_phase_source': f'{mp_mattersim_cache_path} (candidate chemsys only, ensures MatterSim and DFT use same reference phases)'
        },
        'results': results
    }
    ...
    if args.prescreen_results:
        ...
        if prescreen_path.exists():
            ...
            if hull_data:
                ...
                hull_base = f"{output_path.stem}_hull_comparison"
                hull_plot_prefix = output_path.parent / hull_base
                plot_hull_comparison(hull_data, output_prefix=str(hull_plot_prefix))
                
                # Save hull comparison JSON
                hull_comparison_file = output_path.parent / f"{hull_base}.json"
