...
Input:
  - Optional electride-candidate CSV to restrict processing to selected refined structures
  - Structure IDs from user input or all ready structures from a workflow.json
  - In refine mode: start from RELAX_DONE structures and run SPE -> BAND -> DOS
  - In origin mode: start from ELF_DONE structures in VASP-out and run only BAND -> DOS
...
python origin_flow/electronic_flow_manager_origin.py \
  --refine-jobs VASP-out-Boron \
  --source-mode origin \
  --candidate-csv VASP-out-Boron/electride_analysis_candidates-Strict.csv \
  --output-dir ELECTRONIC_JOBS-Boron \
  --db workflow.json
...
def load_candidate_ids_from_csv(csv_path):
...
class WorkflowDatabase:
    ...
    def add_structure(self, struct_id, comp_name, base_dir, initial_state='PENDING', spe_dir=None):
        """Add a new structure to track."""
        base_dir = Path(base_dir)
        resolved_spe_dir = Path(spe_dir) if spe_dir is not None else (base_dir / struct_id / 'SPE')
    ...
    def get_by_state(self, state, structure_ids=None):
        """Get all structures in a specific state."""
        allowed = set(structure_ids) if structure_ids is not None else None
        return [
            sid for sid, sdata in self.data['structures'].items()
            if sdata['state'] == state and (allowed is None or sid in allowed)
        ]
    
    def get_running_count(self, structure_ids=None):
        ...
        allowed = set(structure_ids) if structure_ids is not None else None
        return sum(
            1 for sid, s in self.data['structures'].items()
            if s['state'] in running_states and (allowed is None or sid in allowed)
        )
    
    def get_stats(self, structure_ids=None):
        """Get overall statistics."""
        allowed = set(structure_ids) if structure_ids is not None else None
        states = {}
        total = 0
        for sid, s in self.data['structures'].items():
        ...
        return {
            'total': total,
            'states': states,
            'running': self.get_running_count(structure_ids)
        }
...
"""
    ...
    def monitor_and_submit(self, structures_dict):
        ...
        print("="*70 + "\n")
        sys.stdout.flush()
        active_ids = self.active_structure_ids or list(structures_dict.keys())
        ...
        while True:
            ...
            for struct_id in active_ids:
                self.update_structure_status(struct_id)
            
            running_count = self.db.get_running_count(active_ids)
            ...
            stats = self.db.get_stats(active_ids)
            print("\nStatistics:")
            ...
            # Check if all done
            pending = len(self.db.get_by_state('PENDING', active_ids))
            elf_done = len(self.db.get_by_state('ELF_DONE', active_ids))
            band_done = len(self.db.get_by_state('BAND_DONE', active_ids))
            
            if running_count == 0 and pending == 0 and elf_done == 0 and band_done == 0:
                completed = len(self.db.get_by_state('COMPLETE', active_ids))
                failed = (len(self.db.get_by_state('SC_FAILED', active_ids)) + 
                         len(self.db.get_by_state('PARCHG_FAILED', active_ids)) + 
                         len(self.db.get_by_state('ELF_FAILED', active_ids)) + 
                         len(self.db.get_by_state('BAND_FAILED', active_ids)) + 
                         len(self.db.get_by_state('PDOS_FAILED', active_ids)))
                total = stats['total']
    ...
    def initialize_structures(self, refine_jobs_dir, output_dir, structure_ids=None, candidate_csv=None, source_mode='refine'):
        """Initialize structures from refine or origin workflow."""
        refine_jobs_dir = Path(refine_jobs_dir)
        output_dir = Path(output_dir)
        source_mode = str(source_mode).strip().lower()
        if source_mode not in {'refine', 'origin'}:
            raise ValueError(f"Unsupported source mode: {source_mode}")
        ready_state = 'RELAX_DONE' if source_mode == 'refine' else 'ELF_DONE'
        
        print("="*70)
        print("Electronic Structure Workflow Initialization")
        print("="*70)
        print(f"Source jobs dir: {refine_jobs_dir}")
        print(f"Source mode: {source_mode}")
        print(f"Output directory: {output_dir}")
        print(f"Max concurrent: {self.max_concurrent}")
        print("="*70 + "\n")
        
        # Load source workflow database
        refine_db_path = refine_jobs_dir / 'workflow.json'
        if not refine_db_path.exists():
            print(f"ERROR: Source workflow database not found: {refine_db_path}")
            return {}
        ...
        candidate_ids = None
        if candidate_csv:
            candidate_ids = load_candidate_ids_from_csv(candidate_csv)
            print(f"Loaded {len(candidate_ids)} candidate IDs from CSV: {candidate_csv}")

        selected_ids = list(structure_ids) if structure_ids else None
        if selected_ids and candidate_ids is not None:
            candidate_set = set(candidate_ids)
            before = len(selected_ids)
            selected_ids = [sid for sid in selected_ids if sid in candidate_set]
            print(f"Filtered user-specified structure IDs by candidate CSV: {before} -> {len(selected_ids)}")

        # Filter structures
        if selected_ids:
            print(f"User-specified structure IDs: {len(selected_ids)}")
            selected_id_set = set(selected_ids)
            target_structures = {
                sid: sdata for sid, sdata in refine_db['structures'].items()
                if sid in selected_id_set
            }
        elif candidate_ids is not None:
            candidate_set = set(candidate_ids)
            missing_ids = [sid for sid in candidate_ids if sid not in refine_db['structures']]
            not_relax_done = [
                sid for sid in candidate_ids
                if sid in refine_db['structures'] and refine_db['structures'][sid]['state'] != ready_state
            ]
            print(f"No structure IDs specified - processing {ready_state} electride candidates from CSV")
            print(f"  Candidate IDs in CSV: {len(candidate_ids)}")
            print(f"  Missing from source workflow: {len(missing_ids)}")
            print(f"  Present but not {ready_state}: {len(not_relax_done)}")
            target_structures = {
                sid: sdata for sid, sdata in refine_db['structures'].items()
                if sid in candidate_set and sdata['state'] == ready_state
            }
        else:
            print(f"No structure IDs specified - processing all {ready_state} structures")
            target_structures = {
                sid: sdata for sid, sdata in refine_db['structures'].items()
                if sdata['state'] == ready_state
            }
        ...
        for struct_id, sdata in target_structures.items():
            ...
            if struct_id not in self.db.data['structures']:
                ...
                initial_state = 'PENDING' if source_mode == 'refine' else 'ELF_DONE'
                existing_spe_dir = sdata.get('spe_dir') if source_mode == 'origin' else None
                self.db.add_structure(
                    struct_id,
                    comp_name,
                    output_dir / comp_name,
                    initial_state=initial_state,
                    spe_dir=existing_spe_dir,
                )
        ...
        self.active_structure_ids = sorted(structures_dict.keys())
        
        self.db.data['config'] = {
            'max_concurrent': self.max_concurrent,
            'refine_jobs_dir': str(refine_jobs_dir),
            'output_dir': str(output_dir),
            'candidate_csv': str(Path(candidate_csv).expanduser()) if candidate_csv else None,
            'active_structure_ids': self.active_structure_ids,
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to source workflow directory (REFINE_VASP_JOBS for --source-mode refine, or VASP-out for --source-mode origin)"
    )
    parser.add_argument(
        '--source-mode',
        type=str,
        choices=['refine', 'origin'],
        default='refine',
        help="Input workflow mode: 'refine' starts from RELAX_DONE, 'origin' starts from ELF_DONE and submits only BAND/DOS (default: refine)"
    )
    parser.add_argument(
        '--output-dir',
        type=str,
    ...
    parser.add_argument(
        ...
        nargs='+',
        default=None,
        help="Specific structure IDs to process (default: all ready structures for the selected --source-mode)"
    )
    parser.add_argument(
        '--candidate-csv',
        type=str,
        default=None,
        help="Optional electride-candidate CSV with 'formula' or 'structure_id'; only matching ready structures for the selected --source-mode will be processed"
    )
    parser.add_argument(
        '--db',
        type=str,
