...
Usage:
    python3 refined_flow/postproc_phonon.py --phonon-jobs ./PHONON_JOBS_Boron
            
    python3 refined_flow/postproc_phonon.py --phonon-jobs ./PHONON_Boron_check3 --structure-ids 
    
IDS=$(python3 - <<'PY'
import json
d=json.load(open('/scratch/oridwan/SuperConductorFlow/PHONON_Boron_check3/workflow.json'))
print(" ".join(sid for sid,s in d["structures"].items() if s.get("state")=="PHON_DONE"))
PY
)

python3 /scratch/oridwan/SuperConductorFlow/refined_flow/postproc_phonon.py \
  --phonon-jobs /scratch/oridwan/SuperConductorFlow/PHONON_Boron_check3 \
  --structure-ids $IDS
...
def has_existing_postproc_outputs(phon_dir: Path) -> bool:
    """Return True if the main phonon post-processing plot outputs already exist."""
    output_png = phon_dir / 'phonon_band_dos.png'
    output_pdf = phon_dir / 'phonon_band_dos.pdf'
    return (
        output_png.exists() and output_png.stat().st_size > 0 and
...
def create_force_constants_with_phonopy(
    ...
    """
    # Reuse the exact displacement dataset from the workflow setup when available.
    # This avoids any potential remapping mismatch from regenerating displacements.
    disp_yaml_path = phon_dir / 'phonopy_disp.yaml'
    if disp_yaml_path.exists():
        if phonopy_load is None:
            raise RuntimeError(
                "phonopy_disp.yaml found but phonopy.load is unavailable in this phonopy version; "
                "cannot safely reuse displacement mapping."
            )
        phonon = phonopy_load(
            phonopy_yaml=str(disp_yaml_path),
            factor=VaspToTHz,
            produce_fc=False,
            symmetrize_fc=False,
            is_nac=False
        ...
        # Fallback for older runs without saved displacement YAML.
    ...
    if phonon.supercells_with_displacements is not None:
        n_displacements_expected = len(phonon.supercells_with_displacements)
    elif phonon.dataset and isinstance(phonon.dataset, dict) and 'first_atoms' in phonon.dataset:
        n_displacements_expected = len(phonon.dataset['first_atoms'])
    else:
        raise RuntimeError("Could not determine expected number of displacements from phonopy object")
    if forces.shape[0] != n_displacements_expected:
        raise RuntimeError(
            f"Displacement-force mismatch: expected {n_displacements_expected}, got {forces.shape[0]}"
        )
    ...
    # Enforce translational/permutation consistency to reduce acoustic drift artifacts.
    try:
        phonon.symmetrize_force_constants(show_drift=False)
    except TypeError:
        phonon.symmetrize_force_constants()

    # Optional NAC support for polar materials (LO-TO splitting) if BORN is provided.
    born_path = phon_dir / 'BORN'
    if born_path.exists() and parse_BORN is not None:
        try:
            phonon.nac_params = parse_BORN(phonon.primitive, filename=str(born_path))
            print("    Applied NAC parameters from BORN")
        except Exception as e:
            print(f"    Warning: Failed to apply NAC from BORN: {e}")
    elif born_path.exists() and parse_BORN is None:
        print("    Warning: BORN file found but parse_BORN is unavailable in this phonopy version")
...
def process_structure(
    ...
    if has_existing_postproc_outputs(phon_dir):
        print(f"  Skipping {struct_id}: phonon_band_dos.png/pdf already exist")
        output_summary[struct_id] = {
            'status': 'SKIPPED',
            'reason': 'Post-processing outputs already exist'
        }
        return True
...
def main():
    ...
    parser.add_argument(
        '--phonon-jobs',
        type=str,
        default='./PHONON_JOBS_Boron',
        help="Path to PHONON_JOBS directory (default: ./PHONON_JOBS_Boron)"
    )
    parser.add_argument(
        '--structure-ids',
        type=str,
    ...
    for struct_id, sdata in structures_to_process.items():
        try:
            ...
            status = output_summary.get(struct_id, {}).get('status')
            if status == 'SKIPPED':
                skipped_count += 1
            elif success:
                success_count += 1
    ...
    # Save summary
    output_summary_arg = Path(args.output_summary).expanduser()
    if output_summary_arg.is_absolute():
        summary_path = output_summary_arg
    else:
        # If user already passed "<phonon_jobs_dir_name>/file.json", avoid duplicating
        # to "<phonon_jobs_dir>/<phonon_jobs_dir_name>/file.json".
        rel_parts = [p for p in output_summary_arg.parts if p != "."]
        if rel_parts and rel_parts[0] == phonon_jobs_dir.name:
            summary_path = Path(*rel_parts)
        else:
            summary_path = phonon_jobs_dir / output_summary_arg
