...
class NumpyJSONEncoder(json.JSONEncoder):
    """Custom JSON encoder to handle NumPy types (float32, float64, int32, etc.)"""
...
def get_gpu_memory_usage(device_id=0):
    ...
    Returns:
        tuple: (used_memory_mb, total_memory_mb, free_memory_mb)
    """
    try:
        import subprocess
...
def cleanup_cuda_memory():
    """
    Best-effort cleanup for Python garbage + cached CUDA allocations.
    """
    try:
        import gc
        gc.collect()
...
def relax_structure_mattersim(pmg_struct, calculator, structure_id=None, fmax=0.01, max_steps=500, logfile=None):
    ...
    try:
        ...
        # MatterSim force/stress evaluation uses autograd internally.
        # Do not wrap these calls in torch.no_grad().
    ...
    with open(lock_file, 'w') as lock_f:
        ...
        try:
            ...
            with open(cache_file, 'w') as f:
                json.dump(all_cache_data, f, indent=2, cls=NumpyJSONEncoder)
        ...
        try:
            ...
            try:
                ...
                for mp_id, structure, has_U in mp_phases:
                    ...
                    # Store for caching (convert all numeric types to Python float for JSON serialization)
...
def main():
    ...
    parser.add_argument(
        '--reload-calculator-per-batch',
        action='store_true',
        help="Recreate MatterSimCalculator for each batch (default: reuse one calculator for all batches)"
    )
    parser.add_argument(
        '--mp-cache-dir',
        type=str,
    ...
    print(f"Reload calculator per batch: {args.reload_calculator_per_batch}")
    ...
    try:
        ...
        # Report GPU memory after model loading
        if args.device == 'cuda':
            used, total, free = get_gpu_memory_usage(0)
            if used is not None:
                print(f"  GPU Memory: {used} MiB used / {total} MiB total ({free} MiB free)")
        print()
    except Exception as e:
    ...
    if checkpoint_file.exists():
        ...
        # If no checkpoint exists, try resuming from finalized JSON output.
        # This lets reruns skip structures that were already fully processed.
        if output_file.exists():
            print(f"Found completed output file, loading processed results...")
            try:
                with open(output_file, 'r') as f:
                    output_data_existing = json.load(f)
                results = output_data_existing.get('results', [])
                processed_ids = {r['structure_id'] for r in results if 'structure_id' in r}
                passed = sum(1 for r in results if r.get('passed_prescreening') is True)
                failed = len(results) - passed
                print(f"Loaded {len(results)} processed structures from {output_file.name}")
            except Exception as e:
        ...
        # Open existing database if available; otherwise create a new one
        if db_file.exists():
            print(f"Found existing database: {db_file}")
            db = database_topology(str(db_file))
        else:
    ...
    if not structures_to_process:
        ...
        # Reuse one calculator by default to reduce repeated CUDA model loading churn.
        # Some drivers/libraries keep allocator/context state across loads, which can look
        # like a leak in nvidia-smi when calculators are recreated every batch.
        shared_calc = None
        if not args.reload_calculator_per_batch:
            try:
                ...
                if args.device == 'cuda':
                    used, total, free = get_gpu_memory_usage(0)
                    if used is not None:
                        print(f"[GPU] Shared calculator loaded: {used} MiB used / {total} MiB total ({free} MiB free)\n")
            except Exception as e:
                print(f"ERROR creating shared calculator: {e}")
                return 1
        ...
        for batch_idx in range(n_batches):
            ...
            # Create per-batch calculator only if explicitly requested.
            calc = shared_calc
            if args.reload_calculator_per_batch:
                try:
                    ...
                    if args.device == 'cuda':
                        used, total, free = get_gpu_memory_usage(0)
                        if used is not None:
                            print(f"[GPU] Batch {batch_idx + 1} calculator loaded: {used} MiB used / {total} MiB total ({free} MiB free)\n")
                    ...
                    with open(checkpoint_file, 'w') as f:
                        json.dump(checkpoint_data, f, indent=2, cls=NumpyJSONEncoder)
            ...
            for item in tqdm(batch, desc=f"Batch {batch_idx + 1}", unit="struct"):
                ...
                with open(checkpoint_file, 'w') as f:
                    json.dump(checkpoint_data_temp, f, indent=2, cls=NumpyJSONEncoder)
            ...
            # Clean up calculator after batch only in per-batch mode.
            if args.reload_calculator_per_batch:
                del calc
                if args.device == 'cuda':
                    cleanup_cuda_memory()
            elif args.device == 'cuda':
                # Shared calculator stays alive; release transient cache between batches.
                cleanup_cuda_memory()
            
            print(f"\nBatch {batch_idx + 1} complete.")
            
            # Report GPU memory after batch cleanup
            if args.device == 'cuda':
                used, total, free = get_gpu_memory_usage(0)
                if used is not None:
                    print(f"[GPU] After batch {batch_idx + 1} cleanup: {used} MiB used / {total} MiB total ({free} MiB free)")
        ...
        # Final cleanup for shared calculator
